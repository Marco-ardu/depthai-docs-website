...
*连接 PoE 交换机后，观察设备接口处LED灯是否偶尔闪烁。*

.. note::
    连接设备之后参考以下链接配置设备 :ref:`PoE系列入门必看 <分步教程>`
...
如果您附近没有 PoE 注入器或 PoE 交换机，并且想使用您的 OAK PoE 摄像头，您有 2 个选项：

#. 通过 USB-C 连接相机，使其像 OAK-D（用于电源和通信的 USB-C）一样工作。 您需要将 DIP 开关更改为 0x16 (as on the :ref:`image here <恢复出厂设置>`).
#. 通过 USB-C 为相机供电，同时仍通过以太网与其通信，如下图所示。 您需要在您的计算机上设置静态 IP.

.. figure:: /_static/images/tutorials/poe/oak-poe-usb-power.jpg

  通过 USB-C 连接器为 OAK-D-POE 供电

如果您使用第二个选项，则需要设置计算机的静态 IP. 默认情况下，相机将具有静态
IP ``169.254.1.222``。 因此，您需要将以太网适配器设置为静态 IP，例如。 ``169.254.1.10`` 和网络掩码
``255.255.0.0`` 以使 OAK PoE 摄像头位于同一个 LAN 中并可由 depthai 库搜索.
